...
namespace ProgrammersBlog.Services.Concrete
{
    ...
    {
        public CommentManager(IUow uow, IMapper mapper) : base(uow, mapper)
        {
        }
        public async Task<IDataResult<CommentDto>> GetAsync(int commentId)
        {
            var comment = await Uow.Comments.GetAsync(c => c.Id == commentId);
            if (comment != null)
            {
        ...
        {
            var result = await Uow.Comments.AnyAsync(c => c.Id == commentId);
            if (result)
            {
                var comment = await Uow.Comments.GetAsync(c => c.Id == commentId);
        ...
        {
            var comments = await Uow.Comments.GetAllAsync(null, c => c.Article);
            if (comments.Count > -1)
            {
        ...
        {
            var comments = await Uow.Comments.GetAllAsync(c => c.IsDeleted, c => c.Article);
            if (comments.Count > -1)
            {
                return new DataResult<CommentListDto>(ResultStatus.Success, new CommentListDto
                {
        ...
        {
            var comments = await Uow.Comments.GetAllAsync(c => !c.IsDeleted && c.IsActive, c => c.Article);
            if (comments.Count > -1)
            {
                return new DataResult<CommentListDto>(ResultStatus.Success, new CommentListDto
                {
        ...
        {
            var comment = await Uow.Comments.GetAsync(c => c.Id == commentId, c => c.Article);
            if (comment != null)
            {
                ...
                var updatedComment = await Uow.Comments.UpdateAsync(comment);
                article.CommentCount =
                    await Uow.Comments.CountAsync(c => c.ArticleId == article.Id && !c.IsDeleted);
        ...
        {
            var article = await Uow.Articles.GetAsync(a => a.Id == commentAddDto.ArticleId);
            if (article == null)
            {
            ...
            var addedComment = await Uow.Comments.AddAsync(comment);
        ...
        {
            var oldComment = await Uow.Comments.GetAsync(c => c.Id == commentUpdateDto.Id);
            var comment = Mapper.Map<CommentUpdateDto, Comment>(commentUpdateDto, oldComment);
            comment.ModifiedByName = modifiedByName;
            var updatedComment = await Uow.Comments.UpdateAsync(comment);
            updatedComment.Article = await Uow.Articles.GetAsync(a => a.Id == updatedComment.ArticleId);
            await Uow.SaveAsync();
        ...
        public async Task<IDataResult<CommentDto>> DeleteAsync(int commentId, string modifiedByName)
        {
            ...
            if (comment != null)
            {
                ...
                comment.ModifiedByName = modifiedByName;
                comment.ModifiedDate = DateTime.Now;
                var deletedComment = await Uow.Comments.UpdateAsync(comment);
                article.CommentCount -= 1;
                await Uow.Articles.UpdateAsync(article);
                await Uow.SaveAsync();
        ...
        public async Task<IResult> HardDeleteAsync(int commentId)
        {
            ...
            if (comment != null)
            {
                ...
                {
                    await Uow.Comments.DeleteAsync(comment);
                    await Uow.SaveAsync();
        ...
        {
            var commentsCount = await Uow.Comments.CountAsync();
            if (commentsCount > -1)
            {
        ...
        {
            var commentsCount = await Uow.Comments.CountAsync(c => !c.IsDeleted);
            if (commentsCount > -1)
            {
                return new DataResult<int>(ResultStatus.Success, commentsCount);
            }
